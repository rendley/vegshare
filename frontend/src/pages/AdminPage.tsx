...
const CreateGreenhouseForm = () => {
    ...
    const [createGreenhouse, { isLoading, isSuccess, isError }] = useCreateGreenhouseMutation();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (name.trim() && landParcelId) {
            createGreenhouse({ landParcelId, name }).unwrap().then(() => { setName(''); setLandParcelId(''); setRegionId(''); });
        }
    };

    return (
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 2, p: 2, border: '1px solid grey', borderRadius: 1 }}>
            <Typography variant="h6">3. Создать теплицу</Typography>
            <FormControl fullWidth margin="normal" required>
                <InputLabel>Регион</InputLabel>
                <Select value={regionId} label="Регион" onChange={(e) => setRegionId(e.target.value)} disabled={isLoadingRegions}>
                    {regions?.map(region => <MenuItem key={region.id} value={region.id}>{region.name}</MenuItem>)}
...
const CreatePlotForm = () => {
    ...
    const { data: greenhouses } = useGetGreenhousesByLandParcelQuery(landParcelId, { skip: !landParcelId });
...
const CreateCameraForm = () => {
    ...
    const { data: landParcels } = useGetLandParcelsByRegionQuery(regionId, { skip: !regionId });
    const { data: greenhouses } = useGetGreenhousesByLandParcelQuery(landParcelId, { skip: !landParcelId });
    const { data: plots } = useGetPlotsByGreenhouseQuery(greenhouseId, { skip: !greenhouseId });
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (name.trim() && plotId && rtspPath.trim()) {
            createCamera({ plotId, name, rtsp_path_name: rtspPath }).unwrap().then(() => { 
                setName(''); setRtspPath(''); setPlotId(''); setGreenhouseId(''); setLandParcelId(''); setRegionId(''); 
            });
        }
    };
    ...
    return (
        <Box component="form" onSubmit={handleSubmit} sx={{ mt: 2, p: 2, border: '1px solid grey', borderRadius: 1 }}>
            ...
            <FormControl fullWidth margin="normal" required disabled={!regionId}><InputLabel>Участок</InputLabel><Select value={landParcelId} label="Участок" onChange={(e) => setLandParcelId(e.target.value)}>{landParcels?.map(p => <MenuItem key={p.id} value={p.id}>{p.name}</MenuItem>)}</Select></FormControl>
            <FormControl fullWidth margin="normal" required disabled={!landParcelId}><InputLabel>Теплица</InputLabel><Select value={greenhouseId} label="Теплица" onChange={(e) => setGreenhouseId(e.target.value)}>{greenhouses?.map(g => <MenuItem key={g.id} value={g.id}>{g.name}</MenuItem>)}</Select></FormControl>
            <FormControl fullWidth margin="normal" required disabled={!greenhouseId}><InputLabel>Грядка</InputLabel><Select value={plotId} label="Грядка" onChange={(e) => setPlotId(e.target.value)}>{plots?.map(p => <MenuItem key={p.id} value={p.id}>{p.name}</MenuItem>)}</Select></FormControl>
  ...
  return (
    <div>
      ...
      <Grid container spacing={2} sx={{mt: 1}}>
        ...
        <Grid size={{ xs: 12, md: 6 }}><CreateGreenhouseForm /></Grid>
